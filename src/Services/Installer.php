...
class Installer
{
    ...
    public function checkPermissions()
    {
        ...
        foreach ($direcotries as $key => $directory) {
            ...
            $exts[$key] = ['required' => $directory, 'permission' => $permission, 'value' => ($permission >= $directory ? 1 : 0)];
    ...
    {
        if(session('getJava')) return json_decode(session('getJava'));
